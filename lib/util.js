...
async function applyPatches() {
  ...
  const coreRepoPath = config.projects['courage-core'].dir
...
const util = {
  ...
  updateBranding: () => {
    ...
    const braveComponentsDir = path.join(config.projects['courage-core'].dir, 'components')
    const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
    const braveAppDir = path.join(config.projects['courage-core'].dir, 'app')
    const chromeBrowserResourcesDir = path.join(config.srcDir, 'chrome', 'browser', 'resources')
    const braveBrowserResourcesDir = path.join(config.projects['courage-core'].dir, 'browser', 'resources')
    const braveAppVectorIconsDir = path.join(config.projects['courage-core'].dir, 'vector_icons', 'chrome', 'app')
    const chromeAndroidJavaStringsTranslationsDir = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'strings', 'translations')
    const braveAndroidJavaStringsTranslationsDir = path.join(config.projects['courage-core'].dir, 'browser', 'ui', 'android', 'strings', 'translations')
    ...
    fileMap.add([path.join(config.projects['courage-core'].dir, 'chromium_src', 'chrome', 'installer', 'setup', 'brave_behaviors.cc'),
    ...
    if (config.targetOS === 'android') {
      ...
      const androidResSource = path.join(config.projects['courage-core'].dir, 'android', 'java', 'res')
      const androidResDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res')
      const androidResTemplateSource = path.join(config.projects['courage-core'].dir, 'android', 'java', 'res_template')
      const androidResTemplateDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_template')
      const androidContentPublicResSource = path.join(config.projects['courage-core'].dir, 'content', 'public', 'android', 'java', 'res')
  ...
  updateOmahaMidlFiles: () => {
    ...
    const srcDir = path.join(config.projects['courage-core'].dir, 'win_build_output', 'midl', 'google_update')
  ...
  // Latest version and download url are inserted to cdm header file and courage-core refers it.
...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  signWinBinaries: () => {
    ...
    fs.copySync(path.join(config.outputDir, 'chrome.dll'), path.join(dir, 'chrome.dll'));
     ...
     const core_dir = config.projects['courage-core'].dir
  ...
  copyRedirectCC: () => {
    ...
    const src = path.join(config.projects['courage-core'].dir, 'script', 'redirect-cc.py')
  ...
  lint: (options = {}) => {
    ...
    cmd_options.cwd = config.projects['courage-core'].dir
  ...
  checkoutBraveCore: (options = {}) => {
    Log.progress('Detecting if courage-core exists already.')
    if (!fs.existsSync(config.braveCoreDir)) {
      const braveCoreRef = config.getProjectVersion('courage-core')
      Log.status(`Cloning courage-core [${braveCoreRef}] into ${config.braveCoreDir}...`)
  ...
  gclientSync: (forceReset = false, cleanup = false, braveCoreRef = null, options = {}) => {
    ...
    if (forceReset || braveCoreRef) {
      ...
      args = args.concat(['--revision', 'src/courage@' + braveCoreRef])
      reset = true
    }
    ...
    if (forceReset) {
      ...
      if (cleanup) {
        // temporarily ignored until we can figure out how not to delete src/courage in the process
        // args = args.concat(['-D'])
      }
    }
