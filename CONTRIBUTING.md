...
Since this Project is really new, these will follow at a later date.
I welcome any Pull-Request and welcome people to join this effort.
...
### Help triage issues
One of the easiest ways to help is to [look through our issues tab](https://github.com/courage-browser/courage-browser/issues)
...
* See our [Triage Guidelines page](https://github.com/courage-browser/courage-browser/wiki/Triage-Guidelines) for more info about this process
...
* Having more clear or up-to-date instructions in the README for both [`courage-browser`](https://github.com/courage-browser/courage-browser/blob/master/README.md) and [`courage-core`](https://github.com/courage-browser/courage-core/blob/master/README.md).
* Capturing/updating helpful information [in our wiki](https://github.com/courage-browser/courage-browser/wiki). You'll need to reach out to a Brave team member to request permission - you can do this by creating a new issue or tagging a Brave team member in an existing issue.
...
### Work on the code
* The [repo's wiki](https://github.com/courage-browser/courage-browser/wiki) has instructions for cloning the repo and getting setup on your platform of choice
* Check out the [troubleshooting page](https://github.com/courage-browser/courage-browser/wiki/Troubleshooting) if you get stuck
* Once you're up and running, find an interesting issue to fix. Check out issues labelled with [good first issue](https://github.com/courage-browser/courage-browser/labels/good%20first%20issue)
...
* Submit a [ticket](https://github.com/courage-browser/courage-browser/issues) for your issue if one does not already exist. Please include the Brave version, operating system, and steps to reproduce the issue.
* Fork the repository on GitHub (this might be [`courage-browser`](https://github.com/courage-browser/courage-browser), [`courage-core`](https://github.com/courage-browser/courage-core), or both).
...
* For changes which involve patches, please check out our [Patching Chromium](https://github.com/courage-browser/courage-browser/wiki/Patching-Chromium) guide.
...
* the root project (this repo, [`courage-browser`](https://github.com/courage-browser/courage-browser)), which pulls down all of the Chromium code into `src/`
* [`courage-core`](https://github.com/courage-browser/courage-core) is basically a sub-module (repo in a repo) which is located on disk under the root at `src/courage`

Depending on which you're editing, you'll need to add your fork to the remotes list. By default, `origin` is set to upstream.
For example, here's how GitHub user `bsclifton` would add BOTH remotes `courage-browser` and `courage-core`:
```sh
# root where project is cloned
cd ~/courage-browser/
git remote add bsclifton git@github.com:bsclifton/courage-browser.git
git fetch bsclifton
# root for the `courage-core` repo
cd src/courage
git remote add bsclifton git@github.com:bsclifton/courage-core.git
git fetch bsclifton
```
...
````
    ...
    Fix https://github.com/courage-browser/courage-browser/issues/108
````

* Run the tests by running `npm run test brave_unit_tests` and `npm run test brave_browser_tests`
* JavaScript unit tests can be ran from the `src/courage` directory using `npm run test-unit`


### Keeping your fork up to sync
- Both `courage-browser` and `courage-core` clone themselves with the remote `origin` being upstream, so you can update either using `git pull`.
...
* a descriptive title; this gets used in the release notes ([desktop](https://github.com/courage-browser/courage-browser/blob/master/CHANGELOG_DESKTOP.md) or [android](https://github.com/courage-browser/courage-browser/blob/master/CHANGELOG_ANDROID.md))
...
* Some issues may need to be uplifted to other channels (Dev / Beta / Release). Please see our notes on [uplifting a pull request](https://github.com/courage-browser/courage-browser/wiki/Uplifting-a-pull-request).
